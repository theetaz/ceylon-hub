...
import { isElectionMode, type ChoroplethMode } from "@/stores/layers"
...
export function getChoroplethLabel(mode: ChoroplethMode) {
  ...
  const election = getElection(mode)
  if (election) return `${election.label} — winning party`
  return ""
}
...
export function buildChoroplethFillColor(
  ...
  if (isElectionMode(mode)) {
    const election = getElection(mode)
    if (!election) return fallback
    const winnerProp = winnerPropertyFor(election.id)
    const matchArgs: Array<string> = []
    for (const [code, meta] of Object.entries(election.parties)) {
      matchArgs.push(code, meta.color)
    }
    return [
      "match",
      ["coalesce", ["get", winnerProp], "OTHER"],
      ...matchArgs,
      OTHER_PARTY.color,
